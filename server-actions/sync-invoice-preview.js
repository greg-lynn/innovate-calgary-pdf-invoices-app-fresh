...
function hasNativePreviewPayload(result) {
  if (!result || typeof result !== "object" || result.ok === false) {
    return false;
  }
  const previewPdf = result.previewPdf && typeof result.previewPdf === "object"
    ? result.previewPdf
    : null;
  const preview = result.preview && typeof result.preview === "object"
    ...
    : null;
  return Boolean(
    pickFirst(previewPdf && (previewPdf.pdfDataUrl || previewPdf.pdfBase64 || previewPdf.pdfUrl)) ||
      pickFirst(preview && (preview.pdfDataUrl || preview.pdfBase64 || preview.pdfUrl))
  );
}
...
module.exports = {
  syncInvoicePreviewPayload: async (request = {}, context = {}) => {
    ...
    const strictPreviewRequest = Object.assign({}, normalizedRequest, {
      requestMode: "preview-pdf",
      mode: "preview-pdf",
      prefetchPreviewPdfs: false,
      disablePreviewMode: false,
    });
    const strictResult = normalizePreviewPdfFromResult(
      await syncInvoicesFromSource(strictPreviewRequest, context),
      strictPreviewRequest
    );
    if (hasNativePreviewPayload(strictResult)) {
      return strictResult;
    }
    const prefetchFallbackRequest = Object.assign({}, normalizedRequest, {
      requestMode: "",
      mode: "",
      prefetchPreviewPdfs: true,
      disablePreviewMode: true,
    });
    const fallbackResult = await syncInvoicesFromSource(prefetchFallbackRequest, context);
    return normalizePreviewPdfFromResult(fallbackResult, prefetchFallbackRequest);
  },
};
